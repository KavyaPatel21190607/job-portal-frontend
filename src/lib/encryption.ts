...
export async function decryptMessage(encryptedMessage: string, privateKeyString: string): Promise<string> {
  ...
  try {
    if (!privateKeyString) {
      ...
      return '🔒 [Key not found - Clear browser cache?]';
    }
    ...
    // Message was encrypted with a different key pair
    // This happens when:
    // 1. Browser cache was cleared and keys regenerated
    // 2. Viewing messages on a different device
    // 3. Keys were regenerated by sender/receiver
    return '🔒 [Cannot decrypt - encryption keys changed]';
  }
}
